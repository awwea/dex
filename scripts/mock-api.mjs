...
async function main() {
    ...
    const getHistoryPricesAll = historyPricesCases.map(async (c) => {
        const dict = await getHistoryPrices(c.baseToken, c.quoteToken, c.start, c.end);
    ...
    writeFileSync(join(folder, 'history-prices.json'), JSON.stringify(historyPrices));
    writeFileSync(join(folder, 'simulator-result.json'), JSON.stringify(simulatorResult));
    writeFileSync(join(folder, 'tokenLists.json'), JSON.stringify(tokenListResult));
}
