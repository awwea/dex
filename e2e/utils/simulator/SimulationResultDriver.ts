...
export class SimulationResultDriver {
    ...
    async screenshotSummaryChart() {
        ...
        const mainMenu = new MainMenuDriver(this.page);
        await mainMenu.hide();
        await screenshot(summaryChart, screenshotPath(this.testCase, 'simulator-results-summary'));
        await mainMenu.show();
    }
    ...
    async getAnimationControls() {
        const control = (id: string) => this.page.getByTestId(`animation-controls-${id}`);
        return {
