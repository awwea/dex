...
export class CreateSimulationDriver {
    ...
    async screenshotPriceChart() {
        ...
        await screenshot(priceChart, screenshotPath(this.testCase, 'simulator-input-price'));
    }
    ...
    async selectDay(day: string, monthYear: string) {
        const dayButton = this.page.getByLabel(monthYear).getByText(day, { exact: true });
    ...
    async fillFormSection(direction: Direction, setting: Setting, order: RangeOrder) {
