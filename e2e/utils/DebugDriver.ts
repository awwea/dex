...
import { CreateForkBody, createFork, forkRpcUrl, deleteFork } from './../utils/tenderly';
...
export const setupLocalStorage = async (page: Page, testInfo: TestInfo) => {
    ...
    if (!forkId) throw new Error('Fork should be created before setting the RPC URL');
    ...
    return page.evaluate((storage) => {
        ...
        for (const [key, value] of Object.entries(storage)) {
            localStorage.setItem(`carbon-ethereum-v1.1-${key}`, JSON.stringify(value));
        }
    }, storage);
};
...
export class DebugDriver {
    ...
    async createStrategy(testCase: CreateStrategyTestCase, deps: CreateStrategyDependencies) {
