...
export class EditStrategyDriver {
    ...
    async waitForPage(strategyType: StrategyType, editType: EditType) {
        ...
        const mode = ['deposit', 'withdraw'].includes(editType) ? 'budget' : 'prices';
    ...
    getOverlappingForm() {
        ...
        return {
            ...
            action: (action: 'deposit' | 'withdraw') => form.getByTestId(`action-${action}`),
            budget: () => form.getByTestId('input-budget'),
        };
    }
    ...
    async fillPrice(direction: Direction, setting: Setting, order: MinMax, type: EditType) {
