...
export const useLinearScale = ({ domain, range, pixelsPerTick, domainTolerance = 0 }: Props) => {
    const scale = scaleLinear().domain(calcDomain(domain, domainTolerance)).range(range).nice(-1);
    ...
    const accessor = useCallback((key: keyof SimulatorData) => getAccessor(key, scale), [scale]);
