...
export const ModalImportToken: ModalFC<ModalImportTokenData> = ({ id, data: { address } }) => {
    ...
    return (
        <ModalOrMobileSheet id={id} title="Import Token">
            ...
            {isPending && <div className={cn(blockClasses, 'animate-pulse bg-black')}></div>}
            {isError && (
                <div className={cn(blockClasses, 'bg-error/30 flex items-center justify-center')}>
            ...
            <Button variant="white" fullWidth onClick={onClick} disabled={isPending || isError}>
                Import Token
            </Button>
