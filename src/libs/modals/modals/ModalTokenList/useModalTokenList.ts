...
import { NATIVE_TOKEN_ADDRESS, isGasTokenToHide, nativeToken } from 'utils/tokens';
...
export const useModalTokenList = ({ id, data }: Props) => {
    ...
    const { onClick, excludedTokens = [], includedTokens = [], isBaseToken = false } = data;
    const { closeModal } = useModal();
    const [search, setSearch] = useState('');

    const basePopularTokens = config.popularTokens.base;
    const quotePopularTokens = config.popularTokens.quote;
    const defaultPopularTokens = isBaseToken ? basePopularTokens : quotePopularTokens;
    ...
    const sanitizedTokens = useMemo(
        () =>
            tokens.filter(
                (token) =>
                    (includedTokens.length === 0 || includedTokens.includes(token.address)) &&
    ...
    const _favoriteTokens = useMemo(
        () => favoriteTokens.filter((token) => !excludedTokens.includes(token?.address || '')),
    ...
    const showImportToken = useMemo(() => {
        ...
        return (
            isValidAddress &&
            !filteredTokens.find((token) => token.address.toLowerCase() === search.toLowerCase())
        );
    ...
    const popularTokens = useMemo(() => {
        return defaultPopularTokens
            ...
            .filter((token) => !excludedTokens.includes(token?.address || '')) as Token[];
