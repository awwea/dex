...
export const useModalTradeRouting = ({
    ...
    const [selected, setSelected] = useState<(Action & { isSelected: boolean })[]>(
        ...
        if (approval.approvalRequired) {
            openModal('txConfirm', {
                ...
                eventData: {
                    ...
                    buyToken: target,
                    sellToken: source,
                    valueUsd: getFiatValueSource(data?.totalSourceAmount, true).toString(),
                    approvalTokens: approval.tokens,
    ...
    const onSelect = (id: string) => {
        setSelected((prev) =>
            prev.map((action) =>
                action.id === id ? { ...action, isSelected: !action.isSelected } : action
            )
        );
    };
    ...
    const disabledCTA = !selectedIDs.length || isPending || isError || insufficientBalance;
