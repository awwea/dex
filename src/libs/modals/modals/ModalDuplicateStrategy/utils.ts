...
const replaceDecimal = (key: string, value: any) => {
    if (value instanceof SafeDecimal) return { __type__: 'SafeDecimal', value: value.toString() };
    return value;
};
const reviveDecimal = (key: string, value: any) => {
    if (typeof value === 'object' && value !== null && value['__type__'] === 'SafeDecimal')
...
export const getUndercutStrategy = (strategy: Strategy, undercutDifference: number): Strategy => {
