...
export type GetUserApprovalProps = Pick<Token, 'address' | 'decimals' | 'symbol'> & {
    spender: string;
};
    ...
    return useQueries({
        queries: data.map((t) => ({
            ...
            queryFn: async () => {
                ...
                const allowance = await Token(t.address).read.allowance(user, t.spender);
...
export const useSetUserApproval = () => {
    ...
    return useMutation({
        ...
        > => {
            ...
            const amountWei = isLimited ? expandToken(amount, decimals) : UNLIMITED_WEI;

            const isNullApprovalContract = NULL_APPROVAL_CONTRACTS.includes(address.toLowerCase());
