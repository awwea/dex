...
import { validAddress, validLiteral, validNumber, validateSearchParams } from 'libs/routing/utils';
...
export const createStrategyPage = new Route({
    ...
    validateSearch: validateSearchParams<CreateSelectToken>({
        base: validAddress,
        quote: validAddress,
        strategyOption: validLiteral(['buy-limit', 'range-order', 'two-ranges', 'overlapping']),
    }),
});
