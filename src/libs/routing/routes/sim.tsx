...
import { getLastVisitedPair, validAddress, validBoolean, validNumber } from 'libs/routing/utils';
...
export const simulatorInputRecurringRoute = new Route({
    ...
    validateSearch: (search: Record<string, unknown>): StrategyInputSearch => {
        const sellMax = v.is(validNumber, search.sellMax) ? roundSearchParam(search.sellMax) : '';
        const sellMin = v.is(validNumber, search.sellMin) ? roundSearchParam(search.sellMin) : '';
        const sellBudget = v.is(validNumber, search.sellBudget)
            ? roundSearchParam(search.sellBudget)
            : '';
        const buyMax = v.is(validNumber, search.buyMax) ? roundSearchParam(search.buyMax) : '';
        const buyMin = v.is(validNumber, search.buyMin) ? roundSearchParam(search.buyMin) : '';
        const buyBudget = v.is(validNumber, search.buyBudget)
            ? roundSearchParam(search.buyBudget)
            : '';
        const sellIsRange = typeof search.sellIsRange === 'boolean' ? search.sellIsRange : true;
        const buyIsRange = typeof search.buyIsRange === 'boolean' ? search.buyIsRange : true;
...
export const simulatorInputOverlappingRoute = new Route({
    ...
    validateSearch: (search: Record<string, unknown>): SimulatorInputOverlappingSearch => {
        const sellMax = v.is(validNumber, search.sellMax) ? roundSearchParam(search.sellMax) : '';
        const buyMin = v.is(validNumber, search.buyMin) ? roundSearchParam(search.buyMin) : '';
...
export const simulatorResultRoute = new Route({
    ...
    validateSearch: (search: Record<string, string>): SimulatorResultSearch => {
        ...
        const type: SimulatorType = search.type === 'overlapping' ? 'overlapping' : 'recurring';
