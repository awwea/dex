...
export class CreateOverlappingDriver {
    ...
    getOverlappingInput() {
        ...
        return {
            ...
            anchor: (anchor: 'buy' | 'sell') => within(form).getByTestId(`anchor-${anchor}`),
            budget: () => within(form).getByTestId('input-budget'),
            marketPriceIndicators: () => within(form).getAllByTestId('market-price-indication'),
    ...
    getUserPriceInput() {
        return {
            ...
            editPrice: () => within(this.getUserPriceForm()).getByTestId('input-price'),
            approveWarning: () =>
                within(this.getUserPriceForm()).getByTestId('approve-price-warnings'),
            confirm: () => within(this.getUserPriceForm()).getByTestId('set-overlapping-price'),
        };
    }
}
