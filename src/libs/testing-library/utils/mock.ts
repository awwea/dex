 ...
 */
export const marketRateHandler = (marketRates: Record<string, Record<string, number>>) => {
    return http.get('**/*/market-rate', ({ request }) => {
        const queryParams = new URL(request.url).searchParams;
        const address = queryParams.get('address')?.toLowerCase();
        if (!address || !marketRates[address]) return new HttpResponse(null, { status: 404 });
 ...
 */
export const tokenListHandlers = Object.keys(tokenListsMock).map((tokenList) => {
