...
const init = async (
    ...
    carbonSDK = new Toolkit(
        api,
        sdkCache,
        decimalsMap ? (address) => decimalsMap.get(address.toLowerCase()) : undefined
    );
    await startDataSync();
    isInitialized = true;
...
const buildOrderBook = async (
    ...
    let results = await carbonSDK.getRateLiquidityDepthsByPair(baseToken, quoteToken, rates);
    ...
    const sortedOrders = orders.sort((a, b) => +new Decimal(a.rate).sub(b.rate).toNumber());
...
const getMiddleRate = (maxBuy: Decimal, maxSell: Decimal): Decimal => {
    if (maxBuy.isFinite() && maxBuy.gt(0) && maxSell.isFinite() && maxSell.gt(0)) {
...
const getOrderBook = async (base: string, quote: string, steps: number): Promise<OrderBook> => {
    ...
    const minBuy = new Decimal(buyHasLiq ? await carbonSDK.getMinRateByPair(base, quote) : 0);
    const maxBuy = new Decimal(buyHasLiq ? await carbonSDK.getMaxRateByPair(base, quote) : 0);
    const minSell = new Decimal(sellHasLiq ? await carbonSDK.getMinRateByPair(quote, base) : 0);
    const maxSell = new Decimal(sellHasLiq ? await carbonSDK.getMaxRateByPair(quote, base) : 0);
    ...
    const step = getStep(stepBuy, stepSell, minBuy, maxBuy, steps, minSell, maxSell);
    ...
    const buy = buyHasLiq
        ? await buildOrderBook(true, base, quote, buyStartRate, step, minBuy, maxBuy, steps)
        : [];

    const sell = sellHasLiq
        ? await buildOrderBook(false, quote, base, sellStartRate, step, minSell, maxSell, steps)
        : [];
...
const sdkExposed = {
    ...
    setOnChangeHandlers: (
        ...
        sdkCache.on('onPairDataChanged', (affectedPairs) => onPairDataChanged(affectedPairs));
        sdkCache.on('onPairAddedToCache', (affectedPairs) => onPairAddedToCache(affectedPairs));
        return;
    },
    ...
    ) => carbonSDK.getTradeDataFromActions(sourceToken, targetToken, isTradeBySource, actionsWei),
