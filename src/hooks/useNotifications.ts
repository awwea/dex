...
import { DispatchNotification, Notification, NotificationStatus } from 'libs/notifications/types';
...
export const useNotifications = () => {
    ...
    const _updateNotificationStatus = (id: string, status: NotificationStatus) => {
        setNotifications((prev) => {
            const newNotifications = prev.map((n) => (n.id === id ? { ...n, status } : n));
