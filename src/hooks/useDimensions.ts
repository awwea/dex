...
const useDimensions = <T extends HTMLElement | null>({
    ...
    const [stateDebounced, setStateDebounced] = useDebounce(initialState, debouncedMs);
    ...
    useEffect(() => {
        ...
        observerRef.current = new ResizeObserver(([entry]) => {
            raf = requestAnimationFrame(() => {
                ...
                const width = boxSize && boxSize[0] ? boxSize[0].inlineSize : contentRect.width;
                const height = boxSize && boxSize[0] ? boxSize[0].blockSize : contentRect.height;

                const isUnchanged =
                    width === prevSizeRef.current.width && height === prevSizeRef.current.height;
