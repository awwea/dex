...
export type StrategyInputDispatch = <T extends InternalStrategyInput, K extends keyof T>(
    key: K,
    value: T[K]
) => void;
...
export const useStrategyInput = ({ searchState }: Props) => {
    ...
    const setSearch = useCallback(
        (search: InternalStrategyInput) => {
            const { buyBudgetError, sellBudgetError, buyPriceError, sellPriceError, ...newSearch } =
                search;
