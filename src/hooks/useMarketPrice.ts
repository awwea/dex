...
export const useMarketPrice = ({ base, quote }: Props) => {
    ...
    const { data: basePriceMap, isPending: isPendingBase } = useGetTokenPrice(base?.address);
    const { data: quotePriceMap, isPending: isPendingQuote } = useGetTokenPrice(quote?.address);
