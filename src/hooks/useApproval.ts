import { GetUserApprovalProps, useGetUserApproval } from 'libs/queries/chain/approval';
...
export const useApproval = (data: ApprovalToken[]) => {
    ...
    const result = useMemo(() => {
        return approvalQuery.map((q, i) => {
            ...
            const newData: ApprovalTokenResult | undefined = q.data && {
                ...
                isNullApprovalToken,
                nullApprovalRequired: isNullApprovalToken && q.data.gt(0) && q.data.lt(amount),
            };
            return {
                ...q,
                data: newData,
    ...
    const approvalRequired = useMemo(() => result.some((x) => x.data?.approvalRequired), [result]);
