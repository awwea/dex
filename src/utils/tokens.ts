...
export const expandToken = (amount: string | number, precision: number) => {
    ...
    return new SafeDecimal(trimmed).times(new SafeDecimal(10).pow(precision)).toFixed(0);
};
...
export const isDifferentGasToken =
    config.network.gasToken.address.toLowerCase() !== NATIVE_TOKEN_ADDRESS.toLowerCase();

export const isGasTokenToHide = (address: string) =>
    isDifferentGasToken && address.toLowerCase() === config.network.gasToken.address.toLowerCase();
