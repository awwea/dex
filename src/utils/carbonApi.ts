import { SimulatorAPIParams, SimulatorReturnNew } from 'libs/queries/extApi/simulator';
import { TokenPriceHistoryResult, TokenPriceHistorySearch } from 'libs/queries/extApi/tokenPrice';
...
const get = async <T>(
    ...
    if (!response.ok) {
        ...
        throw new Error(
            error || `Response was not okay. ${response.statusText} response received.`
        );
    }
    return result as T;
};
...
const carbonApi = {
    ...
    getActivity: async (params: QueryActivityParams, abortSignal?: AbortSignal) => {
