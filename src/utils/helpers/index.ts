...
export const isProduction = window ? window.location.host.includes('carbondefi.xyz') : true;
...
export const shortenString = (string: string, separator = '...', toLength = 13): string => {
    ...
    const end = stringArray.slice(stringArray.length - startEndLength, stringArray.length).join('');
    return start + separator + end;
};

export const wait = async (ms: number = 0) => new Promise((resolve) => setTimeout(resolve, ms));
...
export const buildPairKey = (pair: TradePair) =>
    [pair.baseToken.address.toLowerCase(), pair.quoteToken.address.toLowerCase()].join('-');

export const buildTokenPairKey = (pair: TokenPair) => pair.join('-').toLowerCase();

export const setIntervalUsingTimeout = (callback: Function, interval: number): NodeJS.Timeout => {
...
export const isPathnameMatch = (currentPath: string, href: Pathnames, hrefMatches: Pathnames[]) => {
    // Remove trailing /
    const current =
        currentPath !== '/' && currentPath.endsWith('/') ? currentPath.slice(0, -1) : currentPath;
    if (current === href) return true;
    return hrefMatches.filter((x) => x !== '/').some((x) => current.startsWith(x));
};

export const stringToBoolean = (string: string, fallback: boolean | undefined = false) => {
