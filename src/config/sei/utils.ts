...
import { DragonSwapTokenList, GitRepoResponse, NetworkDataType } from 'config/sei/types';
    ...
    (networkId: string) => (data: Record<string, NetworkDataType[]>) => {
        const networkTokens: Token[] = data[networkId]
            .filter((networkData) => {
                ...
                return networkData.base !== 'usei' && (hasBaseAddress || hasPointerAddress);
            })
            .map((networkData) => {
...
export const tokenDragonswapListParser =
    (repo: string, logoPath: string) => async (data: DragonSwapTokenList) => {
        ...
        const logosFolder = (await getGitFolderContent(gitRepoInfoApi)) as GitRepoResponse[];
        const logos: string[] = logosFolder.filter((r) => isAddress(r.name)).map((r) => r.name);
        ...
        const networkTokens: Token[] = data.tokens.map((d) => {
            return {
                ...
                symbol: d.symbol,
                decimals: d.decimals,
                logoURI: logos.includes(d.address) ? `${assetLogoPath}/${d.address}/logo.png` : '',
            };
        });
        const parsedData: TokenList = {
            id: 'DragonSwap',
