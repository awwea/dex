...
import { isZero, outSideMarketWarning } from 'components/strategies/common/utils';
...
export const EditBudgetDisposablePage = () => {
    ...
    const totalBuyBudget = getTotalBudget(search.editType, order0.balance, search.buyBudget);
    const totalSellBudget = getTotalBudget(search.editType, order1.balance, search.sellBudget);
    ...
    const showBuy = buy || (search.editType === 'withdraw' && !isZero(order0.balance));
    const showSell = !buy || (search.editType === 'withdraw' && !isZero(order1.balance));
