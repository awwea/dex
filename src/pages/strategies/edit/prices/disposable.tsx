...
import { emptyOrder, isZero, outSideMarketWarning } from 'components/strategies/common/utils';
...
export const EditStrategyDisposablePage = () => {
    ...
    const totalBudget = getTotalBudget(search.action ?? 'deposit', initialBudget, search.budget);
                ...
                >
                    ...
                    {buyBudgetChanges && (
                        <p className="text-14 text-white/80">
                            You will withdraw&nbsp;
                            {tokenAmount(buyWithdraw, quote)} from the inactive buy order to your
                            wallet.
                        </p>
                    )}
                    {sellBudgetChanges && (
                        <p className="text-14 text-white/80">
                            You will withdraw&nbsp;
                            {tokenAmount(sellWithdraw, base)} from the inactive sell order to your
                            wallet.
                        </p>
                    )}
                </article>
