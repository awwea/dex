...
export const strategyEvents: CarbonEvents['strategy'] = {
    ...
    strategyBuyLowPriceSet: (strategy) => {
        const gtmStrategyData = prepareGtmBuyStrategyData(strategy);
        gtmStrategyData && sendGTMEvent('strategy', 'strategyBuyLowPriceSet', gtmStrategyData);
    },
    strategyBuyLowBudgetSet: (strategy) => {
        const gtmStrategyData = prepareGtmBuyStrategyData(strategy);
        gtmStrategyData && sendGTMEvent('strategy', 'strategyBuyLowBudgetSet', gtmStrategyData);
    },
    strategySellHighBudgetSet: (strategy) => {
        const gtmStrategyData = prepareGtmSellStrategyData(strategy);
        gtmStrategyData && sendGTMEvent('strategy', 'strategySellHighBudgetSet', gtmStrategyData);
    },
    strategySellHighPriceSet: (strategy) => {
        const gtmStrategyData = prepareGtmSellStrategyData(strategy);
        gtmStrategyData && sendGTMEvent('strategy', 'strategySellHighPriceSet', gtmStrategyData);
    },
    strategySellHighOrderTypeChange: (strategy) => {
        const gtmStrategyData = prepareGtmSellStrategyData(strategy);
        gtmStrategyData &&
            sendGTMEvent('strategy', 'strategySellHighOrderTypeChange', gtmStrategyData);
    },
    strategyCreateClick: (strategy) => {
        const gtmStrategyData = prepareGtmStrategyData(strategy);
        gtmStrategyData && sendGTMEvent('strategy', 'strategyCreateClick', gtmStrategyData);
    },
    ...
    strategyDirectionChange: ({ baseToken, quoteToken, strategySettings, strategyType }) => {
        baseToken &&
            quoteToken &&
            strategySettings &&
            strategyType &&
