...
const tdSecondLine = cn('pb-12 align-top whitespace-nowrap pl-8', 'last:pr-24 last:text-end');
...
const ActivityRow: FC<ActivityRowProps> = ({ activity, hideIds, index }) => {
    ...
    const setAction = () => {
        const actions = searchParams.actions?.includes(activity.action) ? [] : [activity.action];
    ...
    return (
        <>
            <tr className="text-14" style={{ animationDelay: `${index * 50}ms` }}>
                ...
                <td className={tdFirstLine}>{tokenAmount(strategy.buy.budget, quote)}</td>
                <td className={tdFirstLine}>{tokenAmount(strategy.sell.budget, base)}</td>
                <td className={tdFirstLine}>{activityDateFormatter.format(activity.date)}</td>
            </tr>
            <tr className="text-12 text-white/60" style={{ animationDelay: `${index * 50}ms` }}>
                {/* ID */}
                {/* Action Icon */}
...
export const BudgetChange: FC<BudgetChangeProps> = ({ budget, token }) => {
    ...
    const text = value.isNegative() ? tokenAmount(budget, token) : `+${tokenAmount(budget, token)}`;
...
const ActionIcon: FC<ActionIconProps> = ({ action, size }) => {
    ...
    if (action === 'buy') return <IconArrowDown className={cn('rotate-[-60deg]', className)} />;
