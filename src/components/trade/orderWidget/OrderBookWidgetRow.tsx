...
export const OrderBookWidgetRow: FC<Props> = ({ buy, rate, amount, total, base, quote }) => {
    ...
    return (
        <>
            <div className={`${buy ? 'text-buy' : 'text-sell'} overflow-x-hidden py-4`}>
                <Tooltip element={`${prettifyNumber(rate, quoteOptions)} ${quote.symbol}`}>
                    <span>{prettifyNumber(rate, rowOptions)}</span>
                </Tooltip>
            </div>

            <div className="overflow-x-hidden py-4 text-right text-white/80">
                <Tooltip element={`${prettifyNumber(amount, baseOptions)} ${base.symbol}`}>
                    <span>{prettifyNumber(amount, rowOptions)}</span>
                </Tooltip>
            </div>

            <div className="overflow-x-hidden py-4 text-right text-white/80">
                <Tooltip element={`${prettifyNumber(total, quoteOptions)} ${quote.symbol}`}>
