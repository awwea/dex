...
export const useBuySell = ({
    ...
    const maxSourceAmountQuery = useGetMaxSourceAmountByPair(source.address, target.address);
    ...
    }, [buy, getFiatValueSource, provider?.network?.name, source, sourceInput, target]);
    ...
    const checkLiquidity = () => {
        const checkSource = () => new SafeDecimal(sourceInput).gt(maxSourceAmountQuery.data || 0);

        const checkTarget = () => new SafeDecimal(targetInput).gt(liquidityQuery.data || 0);
    ...
    useEffect(() => {
        if (bySourceQuery.data) {
            const { totalTargetAmount, tradeActions, actionsTokenRes, effectiveRate, actionsWei } =
                bySourceQuery.data;
    ...
    useEffect(() => {
        if (byTargetQuery.data) {
            const { totalSourceAmount, tradeActions, actionsTokenRes, effectiveRate, actionsWei } =
                byTargetQuery.data;
    ...
    const errorMsgTarget = useMemo(() => {
        ...
        if (isLiquidityError) {
            return `Available liquidity: ${prettifyNumber(liquidityQuery.data || '0')} ${
                target.symbol
            }`;
        }
    }, [isTargetEmptyError, isLiquidityError, liquidityQuery.data, target.symbol]);
    ...
    const getTokenFiat = useCallback(
        (value: string, query: any) => {
            return new SafeDecimal(value || 0).times(query.data?.[selectedFiatCurrency] || 0);
        },
    ...
    const calcSlippage = useCallback((): SafeDecimal | null => {
        ...
        return null;
    }, [getTokenFiat, sourceInput, sourceTokenPriceQuery, targetInput, targetTokenPriceQuery]);
