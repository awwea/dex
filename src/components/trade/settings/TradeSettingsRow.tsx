...
const buttonErrorClasses = 'border-error text-error hover:border-error focus:text-error';
const inputClasses =
...
export const TradeSettingsRow: FC<{
    ...
    const handleOnBlur = ({ target: { value } }: ChangeEvent<HTMLInputElement>) => {
        if (!isValidValue(item.id, value)) {
            isError && setIsError(false);
            internalValue && setInternalValue('');
    ...
    const handleOnInputChange = ({ target: { value } }: ChangeEvent<HTMLInputElement>) => {
    ...
    useEffect(() => {
        warningMessage && carbonEvents.trade.tradeWarningShow({ message: warningMessage });
    }, [warningMessage]);
        ...
        <div>
            ...
            <div className="mt-10 grid grid-cols-4 gap-10">
                {item.presets.map((value) => (
                    <Button
                        ...
                        className={cn(buttonClasses, item.value === value && buttonActiveClasses)}
                    >
                        {item.prepend}
                        {value}
                        {item.append}
            ...
            {warningMessage && (
                <div className="mt-15 text-12 font-weight-500 text-warning flex">
                    ...
                    <span className={`ml-5 ${isError ? 'text-error' : ''}`}>{warningMessage}</span>
                </div>
            )}
        </div>
    );
