...
import { getHandleSelector, getSelector } from 'components/simulator/input/d3Chart/utils';
import { getMaxBuyMin, getMinSellMax } from 'components/strategies/overlapping/utils';
import { ScaleLinear } from 'd3';
import { useCallback, useEffect, useMemo, useRef } from 'react';
import { formatNumber, prettifyNumber } from 'utils/helpers';

type Props = Pick<D3ChartCandlesticksProps, 'prices' | 'onPriceUpdates' | 'dms' | 'onDragEnd'> & {
...
export const D3ChartOverlapping = (props: Props) => {
    const { dms, yScale, prices, marketPrice, onPriceUpdates, onDragEnd, spread } = props;
    ...
    const onDragEndBuy = useCallback(() => {
        ...
        const y = Number(getSelector(getHandleSelector('buy', 'line1')).select('line').attr('y1'));
        const buyMin = yScale.invert(y).toString();
        const { sellPriceLow } = calcPrices(buyMin, prices.sell.max);
        onDragEnd?.({
            buy: { min: buyMin, max: prices.buy.max },
    ...
    const onDragEndSell = useCallback(() => {
        ...
        const y = Number(getSelector(getHandleSelector('sell', 'line1')).select('line').attr('y1'));
        const sellMax = yScale.invert(y).toString();
        const { buyPriceHigh } = calcPrices(prices.buy.min, sellMax);
        onDragEnd?.({
            buy: { min: prices.buy.min, max: buyPriceHigh },
    ...
    return (
        <>
            ...
            />
            <D3ChartOverlappingRangeGroup dms={dms} onDrag={onDragRect} onDragEnd={onDragEndRect} />
