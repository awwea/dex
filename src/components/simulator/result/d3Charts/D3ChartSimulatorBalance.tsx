...
export const D3ChartSimulatorBalance = ({
    ...
    const xScale = scaleBand().domain(['base', 'quote']).range([0, dms.boundedWidth]).padding(0.1);
...
const Bar = ({ id, xScale, yScale, value, dms, percentage, label, symbol }: BarProps) => {
    const minValue = 2;
    const x = xScale(id);
    ...
    return (
        ...
        >
            <circle
                ...
                className="stroke-background-800"
            />
            <ToggleEye visible={isVisible} transform="translate(5, 5)" className="text-white" />
        </g>
    );
};
