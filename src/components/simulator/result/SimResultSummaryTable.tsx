...
export const SimResultSummaryTable = ({ baseToken, quoteToken, buy, sell }: Props) => {
    ...
    return (
        <article className="grid grid-cols-[auto,auto,auto,auto] grid-rows-2 items-center justify-evenly gap-x-8 gap-y-4">
            ...
            <span data-testid="table-sell-order-rates">
                {isSellLimitOrder ? sellMin : `${sellMin} - ${sellMax}`} {quoteSymbol} per{' '}
                {baseSymbol}
            </span>
            ...
            <span data-testid="table-buy-order-rates">
                {isBuyLimitOrder ? buyMin : `${buyMin} - ${buyMax}`} {quoteSymbol} per {baseSymbol}
            </span>
            <span className="text-white/40">|</span>
