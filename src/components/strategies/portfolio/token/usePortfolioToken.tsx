...
export const usePortfolioToken = ({ address, strategies, isPending: _isPending }: Props) => {
    ...
    const tableData = useMemo<PortfolioTokenData[]>(() => {
        ...
        const unsorted = selectedToken.strategies.map((strategy) => {
            ...
            const handleOrder = ({ balance }: Order) => {
                ...
                share = new SafeDecimal(value).dividedBy(selectedToken.value).times(100);
            };
        ...
        return sortObjectArray(unsorted, 'share', (a, b) => (a.share.gt(b.share) ? -1 : 1));
