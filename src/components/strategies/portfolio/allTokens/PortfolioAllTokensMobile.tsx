...
export const PortfolioAllTokensMobile: FC<Props> = ({ data, isPending, getHref }) => {
    const {
    ...
    return (
        <div className={cn('space-y-20')}>
            {isPending
                ? Array.from({ length: 3 }).map((_, i) => <PortfolioMobileCardLoading key={i} />)
                : data.map((value, i) => (
                      <PortfolioMobileCard
                          key={i}
                          index={i}
                      ...
                      >
                          ...
                          <CardSection title="Share" value={buildPercentageString(value.share)} />
