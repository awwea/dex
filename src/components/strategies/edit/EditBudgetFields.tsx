...
export const EditStrategyBudgetField: FC<Props> = ({
    ...
    const insufficientBalance =
        balance.data && new SafeDecimal(max).lt(budget) ? 'Insufficient balance' : '';

    const setBudget = (budget: string) => setOrder({ budget });
    const setMarginalPrice = (marginalPrice: string) => setOrder({ marginalPrice });
        ...
        >
            <header className="flex items-center justify-between">
                <h3 id={titleId} className="text-18 flex items-center gap-8">
                    {`${editType === 'withdraw' ? 'Withdraw' : 'Deposit'}`} {buy ? 'Buy' : 'Sell'}{' '}
                    Budget
                </h3>
                <Tooltip
