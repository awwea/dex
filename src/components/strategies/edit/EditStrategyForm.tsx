...
import { toStrategyEventParams, useStrategyEvent } from '../create/useStrategyEventData';
...
export const EditStrategyForm: FC<Props> = (props) => {
    ...
    const strategyEventData = useStrategyEvent(toStrategyEventParams(strategy, strategyType));
    ...
    const isLoading = (approval.isPending && !!user) || isPending || isProcessing || isDeleting;
    const loadingChildren = getStatusTextByTxStatus(isPending, isProcessing || isDeleting);
    ...
    const submit = (e: FormEvent<HTMLFormElement>) => {
        ...
        if (!hasNoBudget(strategy) && isZero(orders.buy.budget) && isZero(orders.sell.budget)) {
    ...
    return (
        ...
        >
            ...
            <Button type="reset" disabled={isLoading} variant="secondary" size="lg" fullWidth>
                Cancel
            </Button>
        </form>
    );
