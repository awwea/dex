...
export const EditStrategyLayout: FC<Props> = (props) => {
    ...
    return (
        <div className={`flex flex-col gap-20 p-20 ${showGraph ? '' : 'md:justify-self-center'}`}>
            <header className="flex items-center gap-16">
                ...
                <h1 className="text-24 font-weight-500 flex-1">{titleByType[editType]}</h1>
                {!showGraph && (
                    <button
                        onClick={() => {
            ...
            <div className="flex flex-col gap-20 md:flex-row-reverse md:justify-center">
                {showGraph && <StrategyGraph setShowGraph={setShowGraph}>{graph}</StrategyGraph>}
                {children}
            </div>
        </div>
    );
