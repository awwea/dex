...
export const EditOverlappingBudget: FC<Props> = (props) => {
    ...
    useEffect(() => {
        if ((anchor === 'buy' && aboveMarket) || (anchor === 'sell' && belowMarket)) {
        ...
        <OverlappingMarketPriceProvider marketPrice={+marketPrice}>
            {hasNoBudget(strategy) && (
                <article className="rounded-10 bg-background-900 flex flex-col gap-16 p-20">
                    ...
                    <Warning>
                        Since the strategy had no budget, it will use the current market price to
                        readjust the budget distribution around.
                    </Warning>
                ...
                >
                    <hgroup>
                        ...
                        <p className="text-14 text-white/80">
                            Following the above {editType} amount, these are the changes in budget
                            allocation
                        </p>
                    </hgroup>
                    <BudgetDistribution
