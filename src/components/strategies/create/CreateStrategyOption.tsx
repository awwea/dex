        ...
        <>
            ...
            >
                ...
                {items.map(({ id, label, description, benefits, isRecommended }) => (
                    <article
                        ...
                        className={cn('flex flex-col gap-16', id === selectedId ? '' : 'hidden')}
                    >
                        <hgroup>
                            <h3 className="text-14 font-weight-500 mb-8">{label}</h3>
                            <p className="text-12 text-white/70">{description}</p>
                        </hgroup>
                        <h4 className="text-12 font-weight-500 hidden">Benefits</h4>
                        {benefits.map(({ summary, details }, i) => (
                            ...
                            >
                                ...
                                <p className="pl-22 text-10 md:text-12 text-white/60">{details}</p>
                            </details>
                        ))}
                        {isRecommended && (
                            <p id={'legend-' + id} className="text-12 flex gap-8 text-white/40">
                                <span className="bg-primary-dark text-primary rounded p-1">
                                    <IconStar aria-hidden className="size-14" />
                                </span>
                                DFI Signature Feature
                            </p>
                        )}
                    </article>
                ))}
            ...
            <Link
                ...
                className={cn(
                    ...
                    selectedItem ? '' : 'pointer-events-none cursor-not-allowed opacity-40'
                )}
                onClick={() => {
