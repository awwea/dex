...
export const StrategyBlockBuySell: FC<{
    ...
    return (
        <article className={cn('flex flex-col gap-4 p-16', active ? '' : 'opacity-50', className)}>
            {buy ? (
                <header className="flex items-center gap-4">
                    <h4 className="text-12 text-buy">Buy {token.symbol}</h4>
                    {hasFiatValue && (
                        <Tooltip element={buyTooltip} iconClassName="size-10 text-white/60" />
                    )}
                    {!hasFiatValue && (
                        <Tooltip
                            element={
                ...
                <header className="flex items-center gap-4">
                    ...
                    {hasFiatValue && (
                        <Tooltip element={sellTooltip} iconClassName="size-10 text-white/60" />
                    )}
                    {!hasFiatValue && (
                        <Tooltip
                            element={
            ...
            <Tooltip
                element={
                    <span className="inline-flex items-center gap-4">
                        <LogoImager className="size-16" src={otherToken.logoURI} alt="token" />
            ...
            <p data-testid={`${testIdPrefix}-budget-fiat`} className="text-12 text-white/60">
