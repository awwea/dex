...
export const getTooltipTextByStatus = (isExplorer: boolean | undefined, status: StrategyStatus) => {
    if (status === 'active') {
        return isExplorer ? 'This strategy is currently active and ready to process trades.' : '';
...
const tooltipTextByStrategyEditOptionsId = {
    duplicateStrategy: 'Create a new strategy with the same details or undercut it',
    deleteStrategy: 'Delete the strategy and withdraw all associated funds to your wallet',
    ...
    editPrices: 'Edit the prices of your buy/sell orders within an existing strategy',
...
export const getTooltipTextByStrategyEditOptionsId = (isOwn: boolean | undefined) => {
    return {
        ...tooltipTextByStrategyEditOptionsId,
        ...(!isOwn && {
            manageNotifications: 'Get notified when someone trades against this strategy',
        }),
    };
};
