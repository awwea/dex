import { isOverlappingStrategy, isPaused } from 'components/strategies/common/utils';
...
import { StrategyEditOptionId, getTooltipTextByStrategyEditOptionsId } from './utils';
            ...
            <ul role="menu" data-testid="manage-strategy-dropdown">
                {items.map((item) => {
                    if (typeof item === 'number') {
                        return <hr key={item} className="border-1  border-background-700 my-10" />;
                    }
...
export const ManageButtonIcon = forwardRef<HTMLButtonElement, ManageButtonProps>(
...
const ManageItem: FC<{
    ...
    if (tooltipText) {
        return (
            <Tooltip disabled={belowBreakpoint('md')} element={tooltipText} interactive={false}>
                <li role="none">
                    <Content />
                </li>
            </Tooltip>
