...
const isSmallRange = ({ order0, order1 }: Strategy) => {
    const allPrices = new Set([order0.startRate, order0.endRate, order1.startRate, order1.endRate]);
...
export const StrategyGraph: FC<Props> = ({ strategy }) => {
    ...
    const min =
        buy.from && sell.from ? Math.min(buy.from, sell.from) : Math.max(buy.from, sell.from);
    ...
    const getSellPoints = (sellFrom: number, sellTo: number) => {
        if (buyOrderExists) {
            return new Set([
                ...
                `${x(Math.max(sellFrom, buy.from))},${sellFrom > buy.to ? baseline : middle}`,
                `${x(Math.max(sellFrom, buy.from))},${sellFrom < buy.from ? baseline : top}`,
    ...
    return (
        ...
        >
            <defs>
                ...
                <pattern id="base-pattern" width="15" height="25" patternUnits="userSpaceOnUse" />
                <pattern href="#base-pattern" id="buy-pattern">
                    ...
                    <rect x="0" y="0" width="15" height="25" fill="var(--buy)" fillOpacity="0.05" />
                </pattern>
...
export const CurrentPrice: FC<CurrentPriceProps> = ({ currentPrice, token, x }) => {
...
const OrderTooltip: FC<OrderTooltipProps> = ({ strategy, buy }) => {
    ...
    return (
        <article className="text-14 flex flex-col gap-16" data-testid="order-tooltip">
            ...
            {limit && (
                <table className="rounded-8 border-separate border border-white/40">
                    <tbody>
                        <tr>
                            <th className="font-weight-400 p-8 text-start text-white/60">Price</th>
            ...
            {!limit && (
                <table className="rounded-8 border-separate border border-white/40 p-8">
                    <tbody>
                        <tr>
                            <th className="font-weight-400 text-start text-white/60">Min Price</th>
                            <td className="text-end" data-testid="min-price">
                                {startPrice} {quote.symbol}
                            </td>
                        </tr>
                        <tr>
                            <th className="font-weight-400 text-start text-white/60">Max Price</th>
                        ...
                        {!!spread && (
                            <tr>
                                <th className="font-weight-400 text-start text-white/60">Spread</th>
