...
export const BudgetDistribution: FC<Props> = (props) => {
    ...
    const { title, buy, token, initialBudget, withdraw, deposit, balance } = props;
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="text-12 flex justify-between text-white/60">
                <label htmlFor={allocatedId}>
                    Allocated:&nbsp;
                    <span className="text-white">{tokenAmount(allocated || '0', token)}</span>
                </label>
                {!isSimulator && (
                    <label htmlFor={walletId}>
                        Wallet:&nbsp;
                        <span className="text-white">{tokenAmount(balance || '0', token)}</span>
                    </label>
                )}
            </div>
        ...
        if (deposit > balance) {
            return (
                <Warning className="text-12" isError>
                    You should&nbsp;
                    <b className="font-weight-500">deposit {tokenAmount(deposit, token)}</b>
                    {depositFiat && <b>&nbsp;({depositFiat})</b>}
                    &nbsp;from your wallet, but your wallet has insufficient balance. Consider
                    changing token deposit amount or prices.
                </Warning>
            );
        }
        return (
    ...
    if (withdraw) {
        ...
        if (withdraw > initialBudget) {
            return (
                <Warning className="text-12" isError>
                    You should&nbsp;
                    <b className="font-weight-500">withdraw {tokenAmount(withdraw, token)}</b>
                    {withdrawFiat && <b>&nbsp;({withdrawFiat})</b>}
                    &nbsp;from the strategy, but the strategy has insufficient funds. Consider
                    changing token deposit amount or prices.
                </Warning>
            );
        }
