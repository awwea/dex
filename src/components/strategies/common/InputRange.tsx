...
import { cn, formatNumber, roundSearchParam, sanitizeNumber } from 'utils/helpers';
...
export const InputRange: FC<InputRangeProps> = ({
    ...
    const rangeError =
        min && max && +min >= +max && 'Maximum price must be higher than the minimum price';
    ...
    return (
        <>
            ...
            {!!displayError && (
                <Warning isError message={displayError} htmlFor={`${inputMinId} ${inputMaxId}`} />
            )}
            {showWarning &&
                displayWarnings.map((warning, i) => (
                    <Warning key={i} message={warning} htmlFor={`${inputMinId} ${inputMaxId}`} />
                ))}
        </>
    );
};
