...
export const isValidOrder = (order: ValidOrderParams) => {
    return order.isRange ? isValidRange(order.min, order.max) : isValidLimit(order.price);
};
...
export const checkIfOrdersOverlap = (
    ...
    const isSellMinInBuyRange =
        +sell.min < +buy.max && +sell.min >= +buy.min && +sell.min !== 0 && +buy.min !== 0;
...
export const checkIfOrdersReversed = (
    ...
    const isSellRangeSameAsBuyRange =
        sell.min === buy.min && sell.max === buy.max && buy.min !== 0 && buy.max !== 0;

    // 2 range orders
    if (isSellMinBelowBuyMin || isSellMaxBelowBuyMax || isSellRangeSameAsBuyRange) return true;
