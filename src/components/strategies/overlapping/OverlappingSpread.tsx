import { useRef, FC, KeyboardEvent, useState, FocusEvent, ChangeEvent } from 'react';
...
export const OverlappingSpread: FC<Props> = (props) => {
    ...
    return (
        <>
            <div role="group" className="flex items-center gap-10" ref={root} onKeyDown={onKeyDown}>
                {options.map((option) => (
            ...
            {warning && spread && <Warning htmlFor="spread-custom">{warning}</Warning>}
            {spread <= 0 && (
