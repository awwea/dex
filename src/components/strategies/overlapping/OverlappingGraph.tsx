import { FC, MouseEvent as ReactMouseEvent, WheelEvent, useEffect, useRef, useState } from 'react';
...
const clamp = (min: number, value: number, max: number) => Math.min(max, Math.max(value, min));
...
export const OverlappingGraph: FC<Props> = (props) => {
    ...
    const userMarketPrice = props.userMarketPrice ? +props.userMarketPrice : undefined;
    ...
    const steps = new Array(40).fill(null).map((_, i) => left.plus(priceStep.times(i)).toString());
    const stepPoints = Array.from(new Set(steps));

    const priceDistance = maxMean.eq(minMean) ? new SafeDecimal(1) : maxMean.minus(minMean).div(2);
    ...
    const sellTooltipTextLength = Math.max(maxValue.length, maxPercent.length + 1);
    const sellTooltipWidth = sellTooltipTextLength * fontRatio + 4 * padding;
    const baseSellTooltipX = max - sellTooltipWidth / 2;

    const buyTooltipX = Math.min(baseBuyTooltipX, baseSellTooltipX - sellTooltipWidth);
    const buyTooltip = {
        rect: {
            x: buyTooltipX,
    ...
    const drag = (e: MouseEvent) => {
        ...
        if (tooltipPrice) tooltipPrice.textContent = prettifySignedNumber(priceValue / xFactor);
        if (marketPrice) {
    ...
    return (
        <figure className="relative">
            ...
            <svg
                ...
                className={cn(styles.graph, dragging, 'aspect-[400/265] w-full rounded bg-black')}
            ...
            >
                ...
                <defs>
                    ...
                    <pattern href="#base-pattern" id="buy-pattern">
                        ...
                        <use href="#carbonLogo" x={8 * ratio} y={16 * ratio} fill="var(--buy)" />
                        <rect
                            x="0"
                            y="0"
                            width={15 * ratio}
                    ...
                    <pattern href="#base-pattern" id="sell-pattern">
                        ...
                        <use href="#carbonLogo" x={8 * ratio} y={16 * ratio} fill="var(--sell)" />
                        <rect
                            x="0"
                            y="0"
                            width={15 * ratio}
                ...
                <g className={styles.content}>
                    ...
                    <g>
                        ...
                        {prices.map((price) => (
                            <text key={price.toString()} x={price.toString()} {...priceIndicator}>
